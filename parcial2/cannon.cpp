...
//no sirve
//void cannon::defenderBullet(int VO, int angleO)
//{
/*
    int flag = 0;
    bool flag2 = 0;
    ...
    int newVO=46;//VO;
     ...
     t = 0;
     angle = 0;
    Vxoo = newVO*cos((180-newangle)*pi/180);
    Vyoo = newVO*sin((180-newangle)*pi/180);
    for(int V0o = 1; ; V0o += 5){
        for(angle = 0; angle < 90; angle++){
            Vxo=VO*cos((angle)*pi/180);
            Vy0 = V0o*sin((angle)*pi/180);
            x = 0.0;
            y = 0.0;
            x2 = 0.0;
            ...
            for(t = 0; ; t++){
                ...
                x2 = Xd+ Vxoo*(t+1);
                y2 =Yd+ Vyoo*(t+1) -(0.5*G*(t+2)*(t+2));
                x = Xo+Vxo*t;
                y = Yo + Vy0*t -(0.5*G*t*t);

                for(int t2 = t; ;t2++){
                    aux = Xo+Vxo*t2;
                    auy = Yo + Vy0*t2 -(0.5*G*t2*t2);

                    if(sqrt(pow((Xd - aux),2)+pow((Yd - auy),2)) < RO){
                        flag2 = 1;
                        break;
                    }
                    if(auy < 0){
                ...
                if(sqrt(pow((Xo - x2),2)+pow((Yo - y2),2)) < RD){
                    break;
                }
